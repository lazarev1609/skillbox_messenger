...
from app_mess.interface import Ui_MainWindow
...
class ClientProtocol(asyncio.Protocol):
    ...
    def data_received(self, data: bytes):
        print(data)
        decoded = data.decode()
        self.window.textEdit.append(decoded)

    def connection_made(self, transport: transports.Transport):
        self.window.textEdit.append("Успешно подключились, введите login:ВАШ_ЛОГИН")
        self.transport = transport

    def connection_lost(self, exception):
        self.window.textEdit.append("Вы отключены от сервера")
